...
- To change your API key, use **Remove** in the settings and then connect the new one. Removing deletes the Granola webhook endpoint with the key that created it, so nothing is left behind. If Granola rejects that deletion, the settings tell you which endpoint to remove there.
