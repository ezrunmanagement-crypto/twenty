...
3. Paste the key and choose **Connect**. The key is saved as a secret application variable. The app checks it, detects the key type, registers a signed webhook, and queues the last 31 days of notes. **Live sync** shows **Active** once the webhook exists.
4. If live sync shows **Paused** or **Not set up**, use **Resume** or **Set up** to repair the Granola endpoint. The app never re-enables a paused endpoint on its own; the daily catch-up only imports notes updated in the last two days.
...
7. To rotate the key, choose **Replace** next to the API key and connect the new one. Live sync is re-registered with the new key.
