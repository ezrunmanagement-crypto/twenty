...
export class WebhookRateLimitService {
  ...
  >({
    ...
    const { admittedCount, exhausted } =
      await this.usageLimitSpeedService.tryConsumeUpTo({
    ...
    if (isDefined(exhausted)) {
      this.dropThrottledWebhookCalls({
        workspaceId,
        droppedCallCount: webhookEvents.length - admittedCount,
        exhaustedBucket: exhausted,
      });
    }

    return webhookEvents.slice(0, admittedCount);
  }
